...
CREATE TABLE `tarefa` (
  `cod_trf` int(11) NOT NULL,
  `horain_trf` time(6) DEFAULT NULL,
  `horaterm_trf` time(6) DEFAULT NULL,
  `observacao_trf` varchar(45) DEFAULT NULL,
  `tipo_tabalho` int(11) DEFAULT NULL,
  PRIMARY KEY (`cod_trf`),
  KEY `FK_TAREFA_TTRABALHO_idx` (`tipo_tabalho`),
  CONSTRAINT `FK_TAREFA_TTRABALHO` FOREIGN KEY (`tipo_tabalho`) REFERENCES `tipo_trabalho` (`cod_tt`) ON DELETE NO ACTION ON UPDATE NO ACTION
